...
int main(void) {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 2: {char *dst = "";char *src = ""; int dst_size = 0;  printf("Enter dst: "); fgets(dst,sizeof(dst),stdin); printf("Enter src: "); fgets(src,sizeof(src),stdin);  printf("Enter dist_size: ");  fscanf(stdin, "%d", &dst_size); getchar();  str_copy(dst,src,dst_size); printf("string copied: %s\n",dst); break;}
